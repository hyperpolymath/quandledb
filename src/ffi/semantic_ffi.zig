...
// Pointer/length views are owned by the provider until release_* is called.
...
test "empty lookup name is rejected before FFI call" {
    try std.testing.expectError(error.InvalidArgument, semantic_lookup(""));
}

test "empty equivalence name is rejected before FFI call" {
    try std.testing.expectError(error.InvalidArgument, semantic_equivalents(""));
}
